...
def send_welcome(message):
    bot.send_message(message.chat.id, "Здравствуйте! Это бот, который заменяет слова в вашем сообщении.")


@bot.message_handler(content_types=['text'])  # этот обработчик реагирует все прочие сообщения
def change_words(message):
    t = message.text
    bot.send_message(message.chat.id, change(t))
